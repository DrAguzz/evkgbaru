...
import { LayoutDashboard, Calendar, Package, Bike, MapPin, Users, LogOut, ShieldCheck, Route as RouteIcon, BarChart3, Pin, Tag, Settings } from "lucide-react";
...
function AdminShell() {
  ...
  const items: { to: "/admin" | "/admin/bookings" | "/admin/packages" | "/admin/routes" | "/admin/hubs" | "/admin/locations" | "/admin/riders" | "/admin/users" | "/admin/reports" | "/admin/promos" | "/admin/settings"; icon: typeof LayoutDashboard; label: string; exact?: boolean }[] = [
      ...
      <aside className="w-64 shrink-0 bg-slate-950 text-slate-100 border-r border-slate-800 flex-col hidden lg:flex">
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {items.map((it) => {
            ...
            return (
              <Link key={it.to} to={it.to} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition", active ? "bg-primary text-primary-foreground shadow-lg shadow-primary/20" : "text-slate-300 hover:bg-slate-800/60 hover:text-white")}>
        ...
        <div className="p-3 border-t border-slate-800 text-xs text-slate-400">
          ...
          <Button variant="ghost" size="sm" className="w-full justify-start text-slate-200 hover:bg-slate-800 hover:text-white" onClick={async () => { await signOut(); navigate({ to: "/" }); }}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="lg:hidden flex items-center gap-2 p-3 border-b bg-slate-950 text-slate-100 overflow-x-auto">
          {items.map((it) => {
            const active = it.exact ? loc.pathname === it.to : loc.pathname.startsWith(it.to);
            return (
              <Link key={it.to} to={it.to} className={cn("shrink-0 px-3 py-1.5 rounded-full text-xs font-medium", active ? "bg-primary text-primary-foreground" : "bg-slate-800 text-slate-200")}>
                {it.label}
              </Link>
            );
          })}
